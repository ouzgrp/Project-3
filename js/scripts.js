...
.defer(d3.csv,"data/combinedData3.csv")
...
legendGradient = svg.append("g");
  //sample gradients for axes
    ...
    //axes and labels
        ...
        .text("Aid Recieved (millions CAD)");
  ...
  .text("Flow of Foreign Aid (millions CAD)");
}
...
function updateMap(scale){
    ...
    if (scale == aidScaleColor){
        colorAxis = d3.axisBottom(aidScale);              
        colorLegend.text("Aid Recieved (millions CAD)");
...
paths = map_contents.selectAll("path.aid").data(centers)
    ...
    .attr("d", function(x) {
        ...
        x1 = canadacenter[0]; y1 = canadacenter[1];
        x2 = x[0][0]; y2 = x[0][1];
